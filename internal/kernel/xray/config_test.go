...
func TestBuildInbound_VLESS_FallbackString(t *testing.T) {
	nc := &panel.NodeConfig{
		Protocol:   "vless",
		ServerPort: 443,
		TLS:        1,
...
func TestBuildInbound_VLESS_FallbackMap(t *testing.T) {
	nc := &panel.NodeConfig{
		Protocol:   "vless",
		ServerPort: 443,
		TLS:        1,
	...
	if fb[0]["dest"] != "127.0.0.1:8081" {
		t.Errorf("dest = %v, want 127.0.0.1:8081", fb[0]["dest"])
	}
}

func TestBuildInbound_VLESS_FallbackServerPortFields(t *testing.T) {
	nc := &panel.NodeConfig{
		Protocol:   "vless",
		ServerPort: 443,
		TLS:        1,
...
func TestBuildInbound_Trojan_FallbackString(t *testing.T) {
	nc := &panel.NodeConfig{
		Protocol:   "trojan",
		ServerPort: 10443,
		TLS:        1,
...
func TestBuildInbound_Trojan_FallbackMap(t *testing.T) {
	nc := &panel.NodeConfig{
		Protocol:   "trojan",
		ServerPort: 10443,
		TLS:        1,
...
func TestBuildInbound_FallbacksSingleMapWrapped(t *testing.T) {
	nc := &panel.NodeConfig{
		Protocol:   "vless",
		ServerPort: 443,
		TLS:        1,
...
func TestBuildFallbacks_Normalization(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			tls: map[string]interface{}{
				"fallbacks": []interface{}{
					map[string]interface{}{"dest": "127.0.0.1:8080", "xver": "2"},
				},
			},
			wantLen:  1,
		...
		},
		{
			...
			tls: map[string]interface{}{
				"fallbacks": []interface{}{
					map[string]interface{}{"dest": "127.0.0.1:8080"},
		...
		{
			name: "dest wins over server",
			tls: map[string]interface{}{
				"fallback": map[string]interface{}{"server": "10.0.0.1", "dest": "127.0.0.1:8080"},
			},
			wantLen:  1,
			wantDest: []string{"127.0.0.1:8080"},
		},
		{
			name: "map[string]string fallback",
			tls: map[string]interface{}{
				"fallback": map[string]string{"server": "127.0.0.1", "server_port": "8083"},
			},
			wantLen:  1,
			wantDest: []string{"127.0.0.1:8083"},
		},
		{
